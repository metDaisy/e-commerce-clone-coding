...
2. Call `list_projects` when available and use the exact indexed project name. If the repository is not indexed, continue with local exploration; do not index it without explicit approval.
...
5. Use `search_graph` for definitions, implementations, routes, classes, interfaces, callers, and related symbols. Use a narrow query and result limit.
6. Use `search_code` or repository text search for exact literals, configuration keys, test identifiers, error messages, and non-code files.
7. Confirm graph findings with `get_code_snippet` or the checked-out source.
8. Use `trace_path` for callers, callees, dependency paths, data flow, cross-domain paths, and impact analysis. Include tests when coverage is part of the question.
...
- Do not call `delete_project`, ingest traces, update ADRs, or index a repository unless explicitly approved.
- Fall back to local repository exploration when the server, project, index, or capability is unavailable.
