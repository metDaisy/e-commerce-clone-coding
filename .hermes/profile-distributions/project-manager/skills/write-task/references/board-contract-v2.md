...
`board-contract-v1`은 기존 board 재현에만 사용한다. v2 board는 이 문서와
`scripts/validate_board.py`로 판정한다. 새로운 규칙은 새 contract version과 failing regression
test를 먼저 추가한 뒤 다음 graph부터 적용한다.
...
## Planning identity와 stale two-phase
...
`reconciliation` 또는 `investigation` task 하나만 생성한다. Snapshot/planning SHA 불일치는
그 task의 입력이며 자기 자신을 block할 사유가 아니다. Reconciliation 완료 후 확정된 committed
SHA와 gap으로 새 fresh graph를 생성한다. Native unfinished body update에 의존하지 않는다.
...
- check_id: check-snapshot
  ...
  EXPECT: process exit code 0이며 문서가 <literal current_state_sha>를 기준 SHA로 기록한다.
```
...
확인한 결과를 위 field로 comment에 기록한다. Snapshot/planning 불일치 자체로 task를 block하지
않는다. Repository HEAD가 planning SHA와 달라졌거나 독립적인 infrastructure/evidence 장애가
있을 때만 block한다.
