...
flowchart LR
    subgraph Identity["P1·P8·P11 사용자 / 인증 / 판매자"]
        ...
        Seller["<b>Seller</b><br/>id<br/>user<br/>displayName<br/>businessName<br/>contactEmail<br/>contactPhone<br/>status<br/>createdAt<br/>updatedAt"]
    ...
    subgraph Catalog["P2 상품 / 카탈로그"]
        ...
        ProductVariant["<b>ProductVariant</b><br/>id<br/>catalogProduct<br/>displayName<br/>attributes<br/>createdAt<br/>updatedAt"]
    ...
    class User,UserCredential,SocialCredential,SignUpSession,Address,PointHistory,RefreshToken,BlacklistToken,BlacklistUser,Seller,SellerApplication,SellerApplicationReview,SellerStatusHistory,Wishlist identity
    class Category,CatalogProduct,ProductVariant catalog
...
- `SellerApplication`은 신청 이력을 보존하기 위해 User와 `1:N`이고, `PENDING` 신청은 사용자당 하나만 허용한다.
- `Seller`는 승인된 판매자 프로필이므로 User와 `1:0..1`이며, `User.userId`에 연결된 Seller는 하나만 허용한다.
- `SellerApplicationReview`는 신청 승인·거절 이력, `SellerStatusHistory`는 Seller의 정지·재활성화 이력을 보존한다.
- `UserCredential.passwordHash`는 SQL의 `user_credentials.password_hash`에 저장한다.
- `SellerApplication`은 회사명과 연락처를 보관하는 간단한 판매자 신청 원본이다. MVP에서는 사업자등록번호나 증빙 서류를 받지 않는다.
- `SignUpSession`은 정식 User가 생성되기 전의 이메일 OTP·OAuth 가입 흐름을 보관하며, User와 직접 연결하지 않는다.
