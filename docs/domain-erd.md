...
이 문서는 SQL 테이블이 아니라 요구사항과 도메인 객체를 기준으로 작성한 논리 ERD다. 다이어그램에는 현재 구현 객체와 목표 객체를 함께 표시하므로, 구현 여부는 [current-state.md](current-state.md)와 코드·Flyway를 기준으로 판단한다. ERD는 관계를 설명하는 지도이며 API 계약·필드 규칙·현재 구현 상태의 원본이 아니다.
