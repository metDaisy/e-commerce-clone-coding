...
이 문서는 요구사항과 코드에서 같은 용어를 같은 의미로 사용하기 위한 **용어의 단일 기준**이다. 용어 정의와 상태 의미를 관리하지만 API 계약이나 구현 완료 여부를 정의하지 않는다. `현재`로 표시한 내용은 코드·Flyway에 존재하는 사실이고, `목표`로 표시한 내용은 요구사항의 설계 용어다. 상태값의 정확한 영문 상수는 `docs/requirement/index.md`, 현재 코드, Flyway 제약조건의 충돌 여부를 함께 확인한다.
...
| DomainEventEnvelope (목표) | 공개 이벤트의 `eventId`, 버전, aggregate, 추적 ID와 최소 업무 payload를 담는 P6 공통 계약 |
| UserDeactivatedEvent (현재) | P1 User가 계정 비활성화 사실을 발행하고 P11 Auth가 해당 User의 JWT를 무효화하도록 알리는 이벤트 |
| UserRolesChangedEvent (목표) | 역할 집합 변경 사실을 통지하기 위한 목표 이벤트. 현재 코드에서는 확인되지 않는다. |
| EventConsumption (SQL·목표) | 소비자별 `eventId` 처리 결과를 기록해 같은 이벤트의 업무 중복 처리를 막는 P6 모델. 현재 V1 테이블은 존재하지만 소비 도메인은 구현되지 않았다. |
| SagaInstance / SagaStep (SQL·목표) | 여러 도메인 로컬 트랜잭션의 정방향·보상 실행과 상태를 추적하는 P6 모델과 단계. 현재 V1 테이블만 존재한다. |
...
| loginEmail | 로컬 `UserCredential`이 소유하는 로그인 이메일. User 필드나 P1 User API 응답에는 포함하지 않으며, 목표 P11 인증수단 요약에서만 조회한다. 해당 요약 API는 현재 구현에서 확인되지 않는다. |
...
| SignUpSession (목표) | 이메일 OTP 또는 OAuth 인증 후 정식 User를 생성하기 전까지 유지되는 만료 가능한 가입 흐름. 현재 Java 엔티티와 Flyway 테이블에서는 확인되지 않는다. |
...
| Seller (현재·목표 혼재) | 현재 코드의 Seller 프로필은 `PENDING`, `ACTIVE`, `REJECTED`, `SUSPENDED` 상태를 가진다. 목표 계약에서는 User당 승인된 Seller를 하나로 제한하고 `ACTIVE` Seller와 `PRODUCT_MANAGER` 역할을 판매자 기능의 자격으로 함께 사용한다. |
| SellerApplication (목표) | User가 Seller가 되기 위해 제출하는 신청 원본. 현재 Java 엔티티와 Flyway 테이블에서는 확인되지 않는다. |
| SellerApplicationReview (목표) | SellerApplication의 승인·거절 심사 결과와 처리자를 보존하는 불변 이력. 현재 Java 엔티티와 Flyway 테이블에서는 확인되지 않는다. |
| 계정 비활성화 | 주문 이력 보존을 위해 User를 삭제하지 않고 로그인과 사용만 막는 논리적 삭제 상태 |
| FormSignUpTask (현재) | Auth가 User에 프로필 생성을 요청할 때 발행하는 동기 메시지. 완료 이벤트가 아니라 명령 성격이며, `user::signup` Named Interface로 공개된다. |

## 고객 자산

| 용어 | 의미 |
|---|---|
| Address (현재) | P1 User가 소유하는 독립 배송지 리소스. `alias`, 수령인 정보, 주소 본문, `isPrimary`, `lastUsedAt`을 가지며 자체 CRUD와 기본 배송지 불변식을 제공한다. 주문 화면·주소 스냅샷 연계는 목표 범위다. |
...
| Wishlist (SQL·목표) | User와 ProductVariant 사이의 관심 관계. 현재 V1 테이블은 존재하지만 Java 도메인 구현은 확인되지 않았다. |
...
| Tag | `tags` |
| CatalogProductTag | `catalog_product_tags` |
| ProductVariant | `product_variants` |
| Offer | `offers` |
| Inventory | `inventories` |
| Image/Media metadata | `images` |
...
- `FormSignUpTask`는 사용자 프로필 생성 요청 메시지이며 영속 엔티티가 아니다.
- `구매 가능 상태`는 `offers.status`와 `inventories.quantity`로 계산한다.

| 용어 | 의미 |
|---|---|
| Category (현재) | CatalogProduct에 대표 Category 하나로 연결되는 계층형 공용 상품 분류 메타데이터. 현재 `ADMIN` 경로에서 생성·수정하며, 이름·depth(최대 3)·중복·순환을 검증한다. Seller 등록 요청에서 Category를 제안하는 흐름은 목표 범위다. 상품의 전체 분류 경로는 `parentId`를 따라 계산한다. |
...
| ProductVariant (SQL·목표) | CatalogProduct의 메타데이터를 바탕으로 구성된 판매 대상이다. 현재 `product_variants` 테이블은 있으나 Java 도메인·API 구현은 확인되지 않았다. |
| Catalog Registration Request (목표) | `PRODUCT_MANAGER`가 CatalogProduct 또는 ProductVariant의 등록을 관리자에게 요청하는 심사 대상. 현재 Java 엔티티와 Flyway 테이블에서는 확인되지 않는다. |
...
| Offer (SQL·목표) | 승인된 Seller가 특정 ProductVariant를 어떤 가격·판매 상태·판매자 조건으로 판매하는지 나타내는 판매 제안. 현재 `offers` 테이블은 있으나 Java 도메인·API 구현은 확인되지 않았다. |
| Inventory (SQL·목표) | 특정 Offer의 구매 가능 수량과 차감·복원 규칙을 소유하는 재고 정보. 현재 `inventories` 테이블은 있으나 Java 도메인·API 구현은 확인되지 않았다. |
| 구매 가능 상태 (목표) | Inventory 수량을 바탕으로 계산한 `IN_STOCK`, `OUT_OF_STOCK` 등의 표시 상태. `SOLD_OUT`을 상품의 영구 상태로 저장하지 않는다. |
| 기간성 할인 | Offer에 연결된 기본 가격과 시작·종료 시각이 있는 할인 가격. 심화사항에서는 쿠폰·회원가·복수 프로모션으로 확장한다. |
| 적용가격 | 현재 시각과 적용 가능한 가격 정책을 기준으로 계산한 고객 표시 가격 |
| MediaUpload (목표) | P2·P9·P10 리소스에 연결하기 전에 파일 형식·크기·차원·checksum·디코딩을 검증하는 P12의 일회성 업로드 세션. 현재 Java 엔티티·API는 확인되지 않는다. |
| MediaUpload 상태 (목표) | `INITIATED → READY → ATTACHED`가 정상 흐름이며, 취소·검증 실패·만료는 각각 `CANCELLED`, `FAILED`, `EXPIRED` 종단 상태다. |
| MediaStoragePort (목표) | object storage·공개 URL·물리 삭제를 추상화한 P12/common 공개 포트. 현재 포트 구현은 확인되지 않는다. |
...
| MediaAttachment (목표) | Media 파일을 CatalogProduct·Offer·Review에 연결한 도메인 관계와 메타데이터. 현재 SQL의 `images` 테이블은 존재하지만 이 객체·포트 구현은 확인되지 않는다. |
| Image/Media (SQL·목표) | 현재 V1의 `images`는 entity type/id, URL, 대표·정렬 메타데이터를 저장하는 공통 이미지 테이블이다. CatalogProduct·Offer·Review별 업무 API와 실제 저장소 adapter는 목표 범위다. |
| Review (SQL·목표) | 구매와 배송 완료 자격을 검증한 사용자의 평점·텍스트·이미지 리뷰. 현재 V1의 `reviews` 테이블은 있으나 Review 도메인·API는 확인되지 않았다. |
...
이 절의 Cart·Coupon 용어는 요구사항 목표다. 현재 Java 도메인·API는 확인되지 않고 V1 스키마만 존재한다.
...
이 절의 Order·Payment·Delivery 용어는 요구사항 목표다. 현재 Java 도메인·API는 확인되지 않고 V1 스키마만 존재한다.
...
- `ProductVariant.publicationStatus`는 목표 상태이며, 현재는 SQL `product_variants` 테이블만 확인된다.
- 목표 `Offer.status`는 `ACTIVE`, `INACTIVE`, `ARCHIVED`다. 현재 V1 제약조건은 `ACTIVE`, `INACTIVE`만 허용한다.
- 목표에서는 CatalogProduct 또는 ProductVariant가 `ARCHIVED`가 되면 하위 Offer를 `INACTIVE`로 만든다. 이 연계 구현은 확인되지 않았다.
- 목표 `Inventory` 구매 가능 상태는 수량을 기준으로 `IN_STOCK` 또는 `OUT_OF_STOCK`으로 계산한다. 현재 Java 구현은 확인되지 않았다.
- 목표 `Delivery.status`는 `PREPARING`, `SHIPPED`, `IN_TRANSIT`, `DELIVERED`, `CANCELED`다. 현재 V1 제약조건에는 `CANCELED`가 없다.
- `PaymentMethod.methodType`는 `CREDIT_CARD`, `KAKAO_PAY`, `BANK_TRANSFER`다.
- 목표 `Payment.status`는 `PROCESSING`, `SUCCESS`, `FAILED`, `REFUNDED`다. 현재 V1 제약조건은 `SUCCESS`, `FAILED`, `REFUNDED`만 허용한다.
- 목표 `SignUpSession.status`는 이메일 OTP 인증 대기·완료·만료를 구분한다. 현재 Java 엔티티·Flyway 테이블은 확인되지 않는다.
- 목표 `MediaUpload.status`는 `INITIATED`, `READY`, `ATTACHED`, `CANCELLED`, `FAILED`, `EXPIRED`다. 현재 Java 엔티티·API는 확인되지 않는다.
