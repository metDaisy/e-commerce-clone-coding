...
이 문서는 요구사항과 코드에서 같은 용어를 같은 의미로 사용하기 위한 기준이다. `현재`로 표시한 내용은 코드·Flyway에 존재하는 사실이고, `목표`로 표시한 내용은 요구사항의 설계 용어다. 상태값의 정확한 영문 상수는 `docs/requirement/index.md`, 현재 코드, Flyway 제약조건의 충돌 여부를 함께 확인한다.
