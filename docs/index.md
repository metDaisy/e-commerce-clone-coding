...
| 신규 도메인 구현 순서 결정 | [implementation-plan.md](implementation-plan.md) | [current-state.md](current-state.md) |
...
- [implementation-plan.md](implementation-plan.md): 12일 압축 일정과 목표 구현 순서를 정의한다. 현재 완료 상태를 의미하지 않는다.
...
4. `implementation-plan.md`는 계획 문서이므로 코드 존재나 테스트 통과의 증거로 사용하지 않는다.
