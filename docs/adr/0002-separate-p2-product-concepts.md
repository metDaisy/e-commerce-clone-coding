# ADR-0002: P2 상품군·Variant·Offer·재고 책임 분리
...
Amazon과 유사한 상품 화면은 상품 설명, 색상·사이즈 조합, 판매 가격·판매자, 재고 수량을 서로 다른 기준으로 표시한다. 이를 하나의 `Product`에 모두 넣으면 Variant 추가, 복수 판매자, 가격 정책, 재고 차감으로 확장할 때 스키마와 API를 다시 설계해야 한다.
...
- 색상·사이즈 Variant, 복수 Offer, 판매자, 가격 정책으로 확장할 수 있어야 한다.
- 재고 차감과 복원을 독립적으로 검증할 수 있어야 한다.

## Considered Options

### Option A: 하나의 Product에 가격·재고·옵션을 모두 포함

초기 CRUD는 단순하지만 Variant와 복수 판매 Offer를 추가할 때 Product 스키마와 API가 크게 변경된다.

### Option B: CatalogProduct, ProductVariant, Offer, Inventory로 분리

모델은 늘어나지만 각 책임이 명확하고, 초기에는 기본 Variant와 기본 Offer 하나만 생성해 구현 난이도를 제한할 수 있다.
...
- `ProductVariant`는 실제 구매 조합과 SKU를 소유한다. 초기 요구사항에서는 기본 Variant 하나를 생성한다.
- `Offer`는 Variant의 가격, 판매 상태, 상품 상태, 판매자 식별자, 배송·Prime 표시 확장 지점을 소유한다. 초기 요구사항에서는 기본 Offer 하나를 생성한다.
...
- 기본 상품 등록은 단일 Variant·Offer로 단순하게 유지할 수 있다.
- Variant, 복수 판매자, 가격 정책, 재고를 독립적으로 확장할 수 있다.
...
- 장바구니·주문은 `CatalogProduct`가 아니라 구매 가능한 Variant·Offer를 참조한다.
