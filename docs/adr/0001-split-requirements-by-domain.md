...
기존 `docs/requirement.md` 하나에 P1~P6의 비즈니스 규칙, API 계약, 예외, 상태 전이가 함께 들어 있어 특정 도메인의 구현 기준을 찾기 어렵다. 문서가 커질수록 한 도메인의 변경이 다른 도메인 규칙을 실수로 덮어쓸 위험도 커진다. 현재 요구사항은 P1~P12까지 확장되었으므로 문서 진입점과 지원 문서의 동기화 기준도 함께 명확히 해야 한다.
...
- 요구사항이 P1~P12로 확장되어도 도메인별 기준과 공통 계약의 경계를 유지해야 한다.
...
문서 수는 늘어나지만 공통 계약과 도메인 규칙을 분리하고, P1~P12별로 독립적으로 갱신할 수 있다.
...
- `p1/`부터 `p12/`까지의 디렉토리에 도메인별 URI, 요청·응답, 예외, 요구사항, 심화사항을 기록한다.
...
- `architecture.md`, `domain-erd.md`, `domain-glossary.md`, 관련 ADR은 요구사항의 영향을 받는 범위만 참조로 동기화한다. `current-state.md`는 요구사항 목표가 아니라 코드·테스트·Flyway로 확인한 구현 사실만 기록한다.
...
- 이후 P2~P12를 실제 서비스 수준으로 확장할 때 해당 도메인 문서와 영향을 받는 지원 문서만 갱신할 수 있다.
...
- 도메인·상태·관계·모듈 경계가 바뀌면 `docs/index.md`의 기준 문서 순서에 따라 architecture, ERD, glossary, 관련 ADR을 검토한다.
