...
- 구현 기준 Git SHA: `de199195a9011f01343ca729f3fa26f484db1c32`
...
| P2 Catalog & Inventory | 부분 구현 | 카탈로그 상품 생성과 카테고리 목록 조회가 구현되어 있다. 상품 목록·상세 조회, Variant·Offer·Inventory 구현과 관련 테스트는 확인되지 않았다. |
...
| P8 Seller & Marketplace | 부분 구현 | `Seller` 엔티티·저장소·조회 API와 카탈로그 생성 시 판매자/상품 관리자 검증이 존재한다. 판매자 등록·관리 화면/API와 Offer 기반 판매 흐름은 확인되지 않았다. |
...
현재 Controller 선언 기준이며 별도 `/api/v1` context path는 확인되지 않았다.

| Method | Path | 상태 |
|---|---|---|
| POST | `/auth/signup` | 구현 |
| POST | `/auth/refresh` | 구현 |
| POST | `/auth/password/verify` | 구현 |
| POST | `/auth/update` | 구현 |
| GET | `/users/me` | 구현 |
| POST | `/users/update` | 구현 |
| POST | `/catalog-products` | 구현·`PRODUCT_MANAGER`/`ADMIN` 및 활성 판매자 검증 |
| GET | `/categories` | 구현 |
...
- `GET /catalog-products/{catalogProductId}` 상세
...
2. V1 스키마의 P3~P8 테이블 대부분에 대응하는 도메인 모듈과 API가 없다.
3. 카테고리 관리 API는 P7 관리자 기능으로 분리되어야 하며 현재 구현되지 않았다.
4. P2 API와 Catalog 테스트를 추가하고, 이후 P8 판매자·Offer 흐름을 구현한다.
5. 구현 범위가 확장되면 해당 모듈의 테스트와 Modulith 경계 검증을 함께 추가한다.
