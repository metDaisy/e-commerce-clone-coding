...
하나의 Spring Boot 배포 단위 안에서 도메인별 변경과 검증을 지역화하는 모듈러 모놀리스를 구축한다. 모듈은 작은 공개 인터페이스 뒤에 비즈니스 규칙과 저장 세부사항을 숨기고, 향후 MSA 전환 시 모듈 seam을 서비스 seam으로 발전시킬 수 있어야 한다. 이 문서는 장기 구조 원칙과 구조 계약을 정의한다.
...
구조, 모듈 seam, 허용 의존성이 바뀔 때만 이 문서를 갱신한다. 현재 구현 범위의 스냅샷은 [`current-state.md`](current-state.md), 선택 이유는 ADR에 기록한다. 아래에 현재 모듈을 언급하더라도 정확한 구현 사실의 원본은 코드와 `package-info.java`다.
