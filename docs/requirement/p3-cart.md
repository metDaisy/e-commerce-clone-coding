...
- 장바구니의 `total`은 상품 금액 합계이며 배송비는 포함하지 않는다. 배송비는 배송지·배송 그룹이 확정되는 P5 주문 단계에서 계산한다.
- 구매자 응답은 내부 `catalogProductId`·`variantId`를 제외하고 장바구니 표시와 결제 검증에 필요한 정보만 반환한다.
- 조회 시 최신 Offer 가격·상품 보관 여부·구매 가능 재고를 재검증한다. 품절·보관 상품은 `outOfStock`·`unavailable`로 표시하고, P5 주문 금액 미리보기와 주문 생성을 차단한다. 장바구니 조회 자체는 성공시킨다.
