...
4. 이름은 필수이며 API 입력은 공백만으로 구성될 수 없다. 연락처는 선택값이며 입력하면 유효한 전화번호 형식이어야 한다. 이름과 연락처는 활성 User 사이에서 중복될 수 없고, 비활성 User의 값은 재사용할 수 있다.
...
8. P1 보호 API는 Access Token을 `Authorization: Bearer` 헤더로 인증하고 `__Host-REAUTH` 쿠키는 보조적인 재인증 Grant로만 사용한다. 따라서 P1은 CSRF 토큰을 요구하지 않는다. 향후 쿠키만으로 인증하는 구조나 BFF를 도입하면 상태 변경 요청에 CSRF 토큰 또는 동등한 Origin/Fetch Metadata 검증을 적용한다.
...
2. User당 Address 등록 수에는 제한이 없다.
3. 주소를 등록할 때 별칭·수령인 이름·수령인 연락처·우편번호·주소 본문은 필수다.
4. 동일 User는 동일한 주소를 중복 등록할 수 없다. 주소 동일성은 Front에서 앞뒤 공백을 제거한 우편번호와 주소 본문 조합으로 판단하며, 별칭·수령인 정보는 포함하지 않는다.
5. 기본 배송지는 User당 정확히 0개 또는 1개다. 새 기본 배송지를 지정할 때 기존 기본값 해제와 신규 지정을 하나의 트랜잭션으로 처리한다.
6. 첫 번째 Address를 자동으로 기본 배송지로 만들지 않는다. 등록 요청에서 기본 배송지를 명시하거나 별도 기본 배송지 지정 API를 호출해야 한다.
7. 기본 배송지를 삭제하면 `lastUsedAt`이 가장 최근인 주소를 기본 배송지로 승격한다. `lastUsedAt`이 없는 주소끼리는 `createdAt`, `addressId` 내림차순으로 선택하며, 주소가 남지 않으면 기본 배송지는 0개가 된다.
8. Address 목록은 페이지 기반으로 조회하며 기본 `page=0`, `size=20`, 최대 `size=100`을 사용한다. 정렬은 기본 배송지, 최근 사용일, 생성일, 식별자 내림차순이다.
9. Address 수정·삭제·기본 지정은 소유 User의 요청만 허용한다.
10. 주문은 최종 결제 시점에 Address 값을 스냅샷으로 복사한다. 이후 Address 수정·삭제가 이미 생성된 주문의 배송지 스냅샷을 바꾸지 않는다.
...
- 이름과 연락처는 활성 User 간 유일해야 한다. 비활성 User의 값은 재사용할 수 있다.
- `Address.userId`는 하나의 User만 가리키며, User당 Address 수에는 제한이 없다.
- 동일 User의 우편번호·주소 본문 조합은 중복될 수 없다.
