...
이 문서는 **목표 동작**을 위한 도메인 요구사항 인덱스이자 모든 API에 공통으로 적용되는 계약의 원본이다. 현재 구현 여부·완료 여부는 이 문서가 아니라 [`../current-state.md`](../current-state.md)와 코드·테스트·Flyway로 판단한다.
...
데이터가 계속 누적되거나 목록이 자주 변경되는 조회는 도메인 문서에서 커서 기반 조회를 명시한 경우에 사용한다. Review 목록과 구매자 주문 목록은 전체 건수와 페이지 이동이 필요하므로 페이지 기반 조회를 사용하며, P2 CatalogProduct 조회도 관리자·Product Manager의 관리·등록 대상 탐색을 위해 페이지 기반 조회만 사용한다. 설명 키워드 검색은 심화사항으로 둔다.
...
- cursor payload는 Base64URL로 인코딩한다. Base64URL은 인코딩일 뿐 암호화나 변조 방지 기능을 제공하지 않으며, 형식·역직렬화·위치 검증에 실패한 cursor는 `400 INVALID_CURSOR`로 거부한다.
