...
- `Your Orders`와 `Your Payments`는 [P5 Order·Payment](p5-order-payment-delivery.md)가 소유하고, P1은 Account 메뉴에서 해당 화면으로 연결만 한다.
- `Login & security`는 [P11 Auth](p11-auth.md)가 소유하고, P1은 Account 메뉴에서 해당 화면으로 연결만 한다.
...
회원가입 세션, 비밀번호, 이메일 OTP의 발급·검증·재전송은 [P11 Auth](p11-auth.md)가 소유한다. P1 User는 이메일 인증이 완료된 요청을 받아 사용자 프로필을 생성한다.
...
| 메뉴 | 상태 | 책임·동작 |
|---|---|---|
| `Your Orders` | 구현 | [P5](p5-order-payment-delivery.md)의 내 주문 목록·상세·취소 화면으로 이동 |
| `Your Addresses` | 구현 | P1의 주소 목록·등록·수정·삭제·기본 주소 지정 |
| `Your Payments` | 구현 | [P5](p5-order-payment-delivery.md)의 결제수단 등록·조회·삭제 |
| `Login & security` | 구현 | [P11 Auth](p11-auth.md)의 로그인·비밀번호·이메일·인증수단·세션 보안 화면으로 이동 |
