...
- 현재 요구사항은 다중 옵션 선택 UI를 구현하지 않는다. CatalogProduct 등록 요청에는 `variant` 객체 하나만 받고, 심화사항에서 여러 ProductVariant와 옵션 조합으로 확장한다.
...
- 판매자 신청·승인과 판매자 Offer 관리는 P8에서 정의한다. `PRODUCT_MANAGER`는 자신의 CatalogProduct·Offer·재고를 관리하고, `ADMIN`은 플랫폼 운영 목적으로 동일 기능을 수행할 수 있다.
...
- CatalogProduct 등록 요청에는 `sellerId`를 받지 않는다. `PRODUCT_MANAGER`가 등록하면 인증된 `SellerProfile`을 Offer 소유자로 사용하고, `ADMIN`이 등록하면 플랫폼 기본 Offer로 생성한다.
...
등록 요청의 `variant`는 CatalogProduct의 단순 선택 옵션이 아니라 판매할 실제 ProductVariant를 표현한다. 그러므로 `variant.sku`와 `variant.displayName`은 필수이며, 옵션이 없는 CatalogProduct도 다음처럼 입력한다.
