...
|---:|---|---|---|---|---|
| 500 | `CATALOG-010` | Category 트리 조회 실패 | 카테고리를 조회하지 못했습니다. | 없음 | 저장소 원인과 requestId |
...
| HTTP | exceptionCode | 발생 조건 | client message | details | system message |
|---:|---|---|---|---|---|
| 400 | `CATALOG-001` | 이름이 비어 있거나 공백만 포함 | 카테고리 이름을 확인해 주세요. | `field`, `reason` | 입력값을 제외한 검증 원인 |
| 400 | `CATALOG-002` | 부모 지정으로 최대 깊이 3 초과 | 카테고리 깊이는 3단계까지 가능합니다. | `parentId`, `depth` | 계산된 깊이 |
| 404 | `CATALOG-003` | 지정한 부모가 없음 | 상위 카테고리를 찾을 수 없습니다. | 없음 | `parentId`, requestId |
| 409 | `CATALOG-004` | 같은 부모 아래 동일 이름 존재 | 같은 이름의 카테고리가 이미 있습니다. | `parentId`, `name` | 충돌 식별자 |
| 401 | [AUTH-001](../index.md#예외-응답) | — | — | — | — |
| 403 | [ADMIN-001](../p7/p7-admin.md#4-공통-예외) | — | — | — | — |
| 500 | `CATALOG-011` | 저장소 오류 | 요청을 처리하지 못했습니다. | 없음 | 저장소 원인과 requestId |
...
|---:|---|---|---|---|---|
| 400 | `CATALOG-001` | 이름이 비어 있거나 공백만 포함 | 카테고리 이름을 확인해 주세요. | `field`, `reason` | 입력값을 제외한 검증 원인 |
| 400 | `CATALOG-005` | 자기 자신 또는 하위 Category를 부모로 지정 | 카테고리 계층을 확인해 주세요. | 없음 | 순환 후보 ID |
| 400 | `CATALOG-006` | 부모 연결이 순환을 만듦 | 카테고리 계층을 확인해 주세요. | 없음 | 순환 경로 |
| 400 | `CATALOG-002` | 하위 Category 포함 깊이 3 초과 | 카테고리 깊이는 3단계까지 가능합니다. | `categoryId`, `depth` | 계산된 최대 깊이 |
| 404 | `CATALOG-003` | Category 또는 부모가 없음 | 카테고리를 찾을 수 없습니다. | 없음 | 조회 대상 ID |
| 409 | `CATALOG-004` | 같은 부모 아래 이름 중복 | 같은 이름의 카테고리가 이미 있습니다. | `parentId`, `name` | 충돌 식별자 |
| 401 | [AUTH-001](../index.md#예외-응답) | — | — | — | — |
| 403 | [ADMIN-001](../p7/p7-admin.md#4-공통-예외) | — | — | — | — |
| 500 | `CATALOG-012` | 계층 갱신 저장 실패 | 요청을 처리하지 못했습니다. | 없음 | 저장소 원인과 requestId |
...
- `validate`는 이름·부모·깊이·순환·동일 부모 중복을 검증한다.
- 검증 결과는 Category를 생성하거나 이름을 예약하지 않는다.
- Category가 존재하지 않으면 `CATALOG-003`의 원본 의미를 호출 API의 응답 계약에 맞춰 참조한다.
